...
(define-module (johnlepikhin home xplugd)
  ...
  #:use-module (johnlepikhin home xkb)
...
(define-record-type* <home-xplugd-configuration>
  ...
  (keyboard-connected-commands
   home-xplugd-configuration-keyboard-connected-commands
   (default (list (string-append "$HOME/" home-xkb-load-keymap-script-path)))))

(define (generate-default-xplugrc config)
  (let ((kbd-cmds (home-xplugd-configuration-keyboard-connected-commands config)))
    (computed-file
     "xplugrc"
     #~(begin
         (with-output-to-file #$output
           (lambda _
             (display
              (string-append
               "#!/bin/sh\n"
               "# Generated by home-xplugd-service-type\n"
               "\n"
               "TYPE=$1\n"
               "DEVICE=$2\n"
               "STATUS=$3\n"
               ...
               "        case \"$STATUS\" in\n"
               "            connected)\n"
               "                logger -t xplugd \"Keyboard connected: $DESC - applying configuration\"\n"
               #$@(map (lambda (cmd)
                         (string-append
                          "                " cmd " || true\n"))
                       kbd-cmds)
            ...
            (computed-file
             ...
             (with-imported-modules '((guix build utils))
               #~(begin
                   ...
                   (copy-file #$custom-xplugrc #$output)
                   (chmod #$output #o755))))
            (generate-default-xplugrc config))))))
